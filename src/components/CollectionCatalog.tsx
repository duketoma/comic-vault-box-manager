...
export const CollectionCatalog: React.FC<CollectionCatalogProps> = ({
  ...
  const filteredComics = useMemo(() => {
    return comics.filter(c => {
      ...
      if (searchQuery) {
        ...
        if (!matchTitle && !matchIssue && !matchPub && !matchWriter && !matchArtist && !matchTag && !matchEvent && !matchVol && !matchCreator) {
          return false;
        }
      }
  ...
  const hasActiveFilters = selectedGenre !== 'all' || selectedPublisher !== 'all' || selectedAuthor !== 'all' || selectedBoxId !== 'all' || selectedStatus !== 'all' || selectedFormat !== 'all' || selectedEvent !== 'all' || startDate !== '' || endDate !== '';
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3 pt-3">
          ...
          {/* Writer/Artist */}
          <div>
            <label className="block text-[11px] font-semibold text-slate-500 mb-1">Author / Artist</label>
            <select
                  ...
                  <div>
                    <div className="flex items-baseline justify-between gap-1">
                      <h3 className="font-bold text-slate-800 text-xs truncate group-hover:text-slate-900 transition-colors">
                        {comic.title} {comic.volume ? <span className="font-normal text-slate-500 text-[10px]">({comic.volume})</span> : null}
                      </h3>
                    ...
                    <td className="py-3 px-4 font-semibold text-slate-900 flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-bold text-slate-800 hover:text-slate-900">
                          {comic.title} <span className="text-slate-900 font-extrabold">#{comic.issueNumber}</span>
                        </div>
