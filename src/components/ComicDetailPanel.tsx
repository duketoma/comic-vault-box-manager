    ...
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div className="space-y-1 text-xs bg-slate-50 p-2 rounded">
            {comic.writer && (
              <div>
                ...
                <p className="text-slate-700">{comic.writer}</p>
              </div>
            )}
            {comic.artist && (
              <div>
                <span className="text-slate-500 font-medium">Artist:</span>
                <p className="text-slate-700">{comic.artist}</p>
              </div>
            )}
          </div>
        )}
        ...
        <div className="space-y-2">
          <label className="text-xs font-medium text-slate-600">Your Rating</label>
            ...
            {[1, 2, 3, 4, 5].map((star) => (
              <button
                ...
                className={`transition-colors ${
                  star <= rating ? 'text-amber-400' : 'text-slate-300 hover:text-slate-400'
                }`}
              >
        ...
        <div className="space-y-2 text-xs bg-slate-50 p-2 rounded">
          <div>
            ...
            <p className="text-slate-700 capitalize">{comic.readingStatus}</p>
          </div>
          {comic.readCount > 0 && (
            <div>
              <span className="text-slate-500 font-medium">Times Read:</span>
              <p className="text-slate-700">{comic.readCount}</p>
            </div>
          )}
        </div>
        ...
        <div className="space-y-2 text-xs bg-slate-50 p-2 rounded">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-slate-700">{comic.sizeThickness.toFixed(1)}x</span>
          </div>
        </div>
        ...
        {comic.notes && (
          <div className="text-xs bg-amber-50 border border-amber-200 rounded p-2">
            <span className="text-amber-900 font-medium">Notes:</span>
            <p className="text-amber-800 mt-1 line-clamp-3">{comic.notes}</p>
          </div>
        )}
      ...
      {(onDelete || onUpdate) && (
        <div className="border-t border-slate-200 p-3 bg-slate-50 flex gap-2">
          {onDelete && (
            <button
              onClick={onDelete}
              className="flex-1 text-xs px-3 py-2 text-red-600 hover:bg-red-50 rounded transition-colors font-medium flex items-center justify-center gap-1"
            >
